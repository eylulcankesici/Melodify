...
export default function Tracks({Data,Speed,Width,Height, BlackNumbers, KeysPositions,intervalSpeed,options,Player,sound}:TracksProps):ReactElement {
        ...
        arr.map(element =>{
            if(pianoCtx){
                ...
                if(element.wasDetected){
                    ...
                    sound && sound.play_key(element.NoteNumber, element.SoundDuration, element.Velocity);                }
                else{
                    if(BlackNumbers.includes(element.NoteNumber)){
    ...
    return (
        <div>
            ...
            <Piano wh={Height - Height/5} WhiteKeyWidth={Width / 52} height={Height/5} data={[]} sound={sound} />
            <canvas ref={PianoRef} width={Width} height={Height/5 + 5} style={{position:'absolute',zIndex:34,marginTop:Height - Height/5}} />
            <canvas ref={PianoWhiteRef} width={Width} height={Height/5 + 5} style={{position:'absolute',zIndex:32,marginTop:Height - Height/5}} />
